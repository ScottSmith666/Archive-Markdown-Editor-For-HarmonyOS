...
const mainWindow = () => {
  const main2 = new electron.BrowserWindow({
    ...
    webPreferences: {
      preload: !electron.app.isPackaged ? path.join(__dirname, `..${path.sep}..${path.sep}out${path.sep}preload${path.sep}index.js`) : path.join(packedRoot, `out${path.sep}preload${path.sep}index.js`),
      sandbox: false,
  ...
  if (!electron.app.isPackaged && process.env["ELECTRON_RENDERER_URL"]) {
    ...
    main2.loadFile(
      !electron.app.isPackaged ? path.join(__dirname, `..${path.sep}..${path.sep}out${path.sep}renderer${path.sep}index.html`) : path.join(packedRoot, `out${path.sep}renderer${path.sep}index.html`)
    );
  }
  return main2;
};
