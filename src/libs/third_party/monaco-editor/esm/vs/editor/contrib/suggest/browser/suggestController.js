...
let SuggestController = class SuggestController {
    ...
    _alertCompletionItem(item) {
        if (isNonEmptyArray(item.completion.additionalTextEdits)) {
            const msg = localize(1481, "Accepting '{0}' made {1} additional edits", item.textLabel, item.completion.additionalTextEdits.length);
            alert(msg);
        }
    }
...
class TriggerSuggestAction extends EditorAction {
    ...
    constructor() {
        super({
            ...
            label: localize2(1490, "Trigger Suggest"),
...
registerEditorCommand(new SuggestCommand({
    ...
    menuOpts: [{
            ...
            order: 2,
            when: ContextKeyExpr.and(Context.HasFocusedSuggestion, Context.HasInsertAndReplaceRange, Context.InsertMode.isEqualTo('insert')),
            title: localize(1485, "Replace")
        }, {
            menuId: suggestWidgetStatusbarMenu,
            group: 'left',
            order: 2,
            when: ContextKeyExpr.and(Context.HasFocusedSuggestion, Context.HasInsertAndReplaceRange, Context.InsertMode.isEqualTo('replace')),
            title: localize(1486, "Insert")
        }]
}));
...
registerEditorAction(class extends EditorAction {
    constructor() {
        super({
            ...
            label: localize2(1491, "Reset Suggest Widget Size"),
