...
class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    provideWithTextEditor(context, picker, token) {
        ...
        picker.ariaLabel = localize(1351, "Go to line. Type a line number, optionally followed by colon and column number.");
        ...
        const offsetButton = {
            ...
            tooltip: localize(1352, "Toggle Zero-Based Offset"),
        ...
        const updatePickerAndEditor = () => {
            ...
            picker.items = [{
                    ...
                    ariaLabel: lineNumber
                        ? localize(1353, "Go to line {0}, column {1}. Press Enter to navigate.", lineNumber, column || 1)
                        : label,
                }];
    ...
    parsePosition(editor, value) {
        ...
        if (!model) {
            return {
                label: localize(1354, "Open a text editor first to go to a line or an offset.")
            };
        }
        ...
        if (value.startsWith(':')) {
            ...
            if (isNaN(offset)) {
                ...
                return {
                    ...
                    label: this.useZeroBasedOffset ?
                        localize(1355, "Type a character position to go to (from 0 to {0}).", maxOffset - 1) :
                        localize(1356, "Type a character position to go to (from 1 to {0}).", maxOffset)
                };
            }
            else {
                ...
                return {
                    ...pos,
                    inOffsetMode: true,
                    label: localize(1357, "Press 'Enter' to go to line {0} at column {1}.", pos.lineNumber, visibleColumn)
                };
            }
        }
        else {
            ...
            if (parts.length < 1 || isNaN(lineNumber)) {
                return {
                    label: localize(1358, "Type a line number to go to (from 1 to {0}).", maxLine)
                };
            }
            ...
            if (parts.length < 2 || isNaN(column)) {
                return {
                    ...
                    label: parts.length < 2 ?
                        localize(1359, "Press 'Enter' to go to line {0} or enter colon : to add a column number.", lineNumber) :
                        localize(1360, "Press 'Enter' to go to line {0} or enter a column number (from 1 to {1}).", lineNumber, maxColumn)
                };
            }
            ...
            return {
                lineNumber,
                column: realColumn,
                label: localize(1361, "Press 'Enter' to go to line {0} at column {1}.", lineNumber, column)
            };
        }
    }
}
