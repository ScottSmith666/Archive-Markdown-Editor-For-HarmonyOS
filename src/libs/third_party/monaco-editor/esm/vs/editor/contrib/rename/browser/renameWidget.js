...
const CONTEXT_RENAME_INPUT_VISIBLE = new RawContextKey('renameInputVisible', false, localize(1408, "Whether the rename input widget is visible"));
new RawContextKey('renameInputFocused', false, localize(1409, "Whether the rename input widget is focused"));
let RenameWidget = class RenameWidget {
    ...
    beforeRender() {
        ...
        this._label.innerText = localize(1410, "{0} to Rename, {1} to Preview", this._keybindingService.lookupKeybinding(accept)?.getLabel(), this._keybindingService.lookupKeybinding(preview)?.getLabel());
...
class RenameCandidateListView {
    ...
    setCandidates(candidates) {
        ...
        status(localize(1411, "Received {0} rename suggestions", candidates.length));
    }
    clearCandidates() {
...
class InputWithButton {
    ...
    get domNode() {
        if (!this._domNode) {
            ...
            this._inputNode.setAttribute('aria-label', localize(1412, "Rename input. Type new name and press Enter to commit."));
            this._domNode.appendChild(this._inputNode);
            this._buttonNode = document.createElement('div');
            this._buttonNode.className = 'rename-suggestions-button';
            this._buttonNode.setAttribute('tabindex', '0');
            this._buttonGenHoverText = localize(1413, "Generate New Name Suggestions");
            this._buttonCancelHoverText = localize(1414, "Cancel");
