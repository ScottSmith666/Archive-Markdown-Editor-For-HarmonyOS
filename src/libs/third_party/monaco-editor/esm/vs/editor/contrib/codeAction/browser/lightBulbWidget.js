...
let LightBulbWidget = class LightBulbWidget extends Disposable {
    ...
    static _computeLightBulbInfo(state, forGutter, preferredKbLabel, quickFixKbLabel) {
        ...
        let title;
        if (autoRun) {
            title = localize(890, "Run: {0}", actions.validActions[0].action.title);
        }
        else if (actions.hasAutoFix && preferredKbLabel) {
            title = localize(891, "Show Code Actions. Preferred Quick Fix Available ({0})", preferredKbLabel);
        }
        else if (!actions.hasAutoFix && quickFixKbLabel) {
            title = localize(892, "Show Code Actions ({0})", quickFixKbLabel);
        }
        else {
            title = localize(893, "Show Code Actions");
        }
