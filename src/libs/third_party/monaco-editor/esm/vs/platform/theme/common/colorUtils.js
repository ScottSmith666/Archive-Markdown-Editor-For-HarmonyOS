...
class ColorRegistry extends Disposable {
    ...
    registerColor(id, defaults, description, needsTransparency = false, deprecationMessage) {
        ...
        if (needsTransparency) {
            ...
            propertySchema.patternErrorMessage = localize(2046, 'This color must be transparent or it will obscure content');
        }
        this.colorSchema.properties[id] = {
            description,
            oneOf: [
                propertySchema,
                { type: 'string', const: DEFAULT_COLOR_CONFIG_VALUE, description: localize(2047, 'Use the default color.') }
            ]
        };
