...
class ConfigurationRegistry extends Disposable {
    constructor() {
        ...
        this.defaultLanguageConfigurationOverridesNode = {
            id: 'defaultOverrides',
            title: localize(1683, "Default Language Configuration Overrides"),
            properties: {}
        };
    ...
    updateDefaultOverrideProperty(key, newDefaultOverride, source) {
        const property = {
            ...
            description: localize(1684, "Configure settings to be overridden for {0}.", getLanguageTagSettingPlainKey(key)),
    ...
    updateOverridePropertyPatternKey() {
        for (const overrideIdentifier of this.overrideIdentifiers.values()) {
            ...
            const resourceLanguagePropertiesSchema = {
                type: 'object',
                description: localize(1685, "Configure editor settings to be overridden for a language."),
                errorMessage: localize(1686, "This setting does not support per-language configuration."),
                $ref: resourceLanguageSettingsSchemaId,
            };
    ...
    registerOverridePropertyPatternKey() {
        ({
            description: localize(1687, "Configure editor settings to be overridden for a language."),
            errorMessage: localize(1688, "This setting does not support per-language configuration.")});
        this._onDidSchemaChange.fire();
    }
...
function validateProperty(property, schema, extensionId) {
    if (!property.trim()) {
        return localize(1689, "Cannot register an empty property");
    }
    if (OVERRIDE_PROPERTY_REGEX.test(property)) {
        return localize(1690, "Cannot register '{0}'. This matches property pattern '\\\\[.*\\\\]$' for describing language specific editor settings. Use 'configurationDefaults' contribution.", property);
    }
    if (configurationRegistry.getConfigurationProperties()[property] !== undefined && (!extensionId || !EXTENSION_UNIFICATION_EXTENSION_IDS.has(extensionId.toLowerCase()))) {
        return localize(1691, "Cannot register '{0}'. This property is already registered.", property);
    }
    if (schema.policy?.name && configurationRegistry.getPolicyConfigurations().get(schema.policy?.name) !== undefined) {
        return localize(1692, "Cannot register '{0}'. The associated policy {1} is already registered with {2}.", property, schema.policy?.name, configurationRegistry.getPolicyConfigurations().get(schema.policy?.name));
    }
    return null;
}
