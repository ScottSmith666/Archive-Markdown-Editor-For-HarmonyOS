...
> 
> 因此鸿蒙版Archive Markdown Editor不支持将（md、txt）另存为mdz。当新建文件或者mdz文件中存在绝对路径的多媒体语句（如`![...](/path/to/a.jpg)`）时，无法成功保存为mdz文件。
>
> 鸿蒙版Archive Markdown Editor不支持自动嵌入mdz文件并转化为mdz媒体路径的功能，只能操作菜单栏手动嵌入多媒体文件。
