...
>
> 因此鴻蒙版Archive Markdown Editor不支援將（md、txt）另存為mdz。當新建檔案或mdz檔案中存在絕對路徑的多媒體語句（如`![...](/path/to/a.jpg)`）時，無法成功儲存為mdz檔案。
>
> 鴻蒙版Archive Markdown Editor不支援自動嵌入mdz檔案並轉換為mdz媒體路徑的功能，只能操作選單列手動嵌入多媒體檔案。
...
| 操作       | Windows/Linux/Harmony平台    | macOS平台         |
|:---------|:-------------------|:----------------|
| 新建文件     | `Ctrl` `N`         | `⌘` `N`         |
| 開啟文件     | `Ctrl` `O`         | `⌘` `O`         |
| 關閉頁面     | `Ctrl` `W`         | `⌘` `W`         |
