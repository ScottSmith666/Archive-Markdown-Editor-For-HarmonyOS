...
>
> Therefore, the HarmonyOS version of Archive Markdown Editor does not support saving (md, txt) files as mdz. When a new file or an mdz file contains multimedia statements with absolute paths (such as `![...](/path/to/a.jpg)`), it cannot be successfully saved as an mdz file.
>
> The HarmonyOS version of Archive Markdown Editor does not support the function of automatically embedding mdz files and converting them into mdz media paths; multimedia files can only be embedded manually through the menu bar.
